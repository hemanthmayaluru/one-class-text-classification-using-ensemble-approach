...
if __name__ == "__main__":
    ...
    data_ai = arxiv_dataset(train_file_path, 'AI')
    data_bio = arxiv_dataset(test_file_path_bio, 'BIO')
    data_eess = arxiv_dataset(test_file_path_eess, 'EESS')
    #train_data = data_ai[:3000]
    #val_data = data_ai[3000:4000]+data_bio[:300]+data_eess[:300]
    #train_data = data_bio[:400]
    #val_data = data_ai[3000:4000]+data_bio[400:600]+data_eess[:300]
    train_data = data_eess[:400]
    #val_data = data_ai[3000:4000]+data_bio[:300]+data_eess[400:600]
    #test_data = data_bio[300:] + data_eess[300:] + data_ai[4000:]
    #test_data = data_bio[600:] + data_eess[300:] + data_ai[:300]
    #test_data = data_bio[300:] + data_ai[:300] + data_eess[600:]
    val_data = data_ai[3000:4000]+data_eess[400:600]
    test_data = data_bio[:500] + data_eess[600:]
    # Converting the train and test data into dataframe
    train_data_df = pd.DataFrame(train_data, columns = ['text' , 'label'])
    test_data_df = pd.DataFrame(test_data, columns = ['text' , 'label'])
    val_data_df = pd.DataFrame(val_data, columns = ['text' , 'label'])
    
    
    group_list = ['EESS']
    
    #embedding_list = ['use', 'infersent', 'bert', 'glove']
    embedding_list = ['use']
    ...
    filename = '/opt/notebooks/OCSVM_ISF_LOF_USE_Baselines/results_output_new/exp_ensemble_arxiv.txt'
